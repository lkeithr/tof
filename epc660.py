    ...
    def decode_data(self, coded_data, dtype):
        ...
        for data_msg in coded_data:
            ...
            for i in range(0, data_len, 2):
                ...
                data_up.append(p_up)
        if dtype == "distance":
            decoded_data = np.reshape(np.array(data_up), (constant.CAM_RESY, constant.CAM_RESX), order='C')
...
if __name__ == "__main__":    
    ...
    print("Will now try to connect to camera")
    ...
    infoStr = "Camera info is as follows:\n  Part version is " + str(part_ver[0][0]) + ",\n  Server version is " +str(server_ver[0][0]) + ",\n  IC version is " + str(ic_ver[0][0])
    print(infoStr)
    
    """
    # test -- Set modulation to 750, integration time to 1500 us 
    modRate = 2;
    intTime = 700;
    mod_set = cam.set_modulation(modRate)
    int_set = cam.set_int_time(intTime)
    int_time = cam.get_int_time()
    modStr = "Successfully set the modulation to " + str(modRate) + " kHz and the integration time to " + str(intTime) + " us"
    print(modStr)
    
    # Turn the camera modulation on and collect an image
    #mod_set =  cam.set_modulation(1) # modulations: 0/24 MHz, 1/12 MHz, 2/6 MHz, 3/3 MHz, 4/1.5 MHz, 5/0.75 MHz
    #mode_set = cam.select_mode(1) # modes: 0/grayscale, 1/distance color, 2/amplitude color, 6/DCS
    """
    
    # Load the configuration file
    config_filename = "ESPROS_20_or_24MHz_3m5.xml"
    config_set = cam.load_config(config_filename)
    confStr = "Camera configuration set to " + config_filename
    print(confStr) 
    cam.set_int_time(4000)
    print(cam.get_int_time())
    ...
    for cap in range(num_frames):
        ...
        cv2.waitKey(100) # delay between frames in ms 
        print(str(cap))
    
    # Save this data to a file
    pickle.dump(img, open("tofCamTest.dat","wb"))
